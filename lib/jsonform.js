    ...
    'onInsert': function(evt, node) {
      if (global.Picker) {
        ...
        $(node.el).find('#' + escapeSelector(node.id)).spectrum({
          ...
          showAlpha: false,
          allowEmpty: true,
          disabled: node.isReadOnly() ? true : false,
          preferredFormat: "hex",
          showInput: true
        });
      }
  ...
  'imageselect': {
    'template': '<div>' +
      ...
      '<a class="<%= cls.buttonClass %>" data-toggle="dropdown" href="#"<% if (node.value) { %> style="max-width:<%= width %>px;max-height:<%= height %>px"<% } %>>' +
        '<% if (node.value) { %><img class="img-responsive" src="<% if (!node.value.match(/^https?:/)) { %><%= prefix %><% } %><%= node.value %><%= suffix %>" alt="" /><% } else { %><%= buttonTitle %><% } %>' +
      '</a>' +
      '<div class="dropdown-menu" id="<%= node.id %>_dropdown">' +
        '<div>' +
        '<% _.each(node.options, function(key, idx) { if ((idx > 0) && ((idx % columns) === 0)) { %></div><div><% } %><a class="btn<% if (buttonClass) { %> <%= buttonClass %><% } %>" style="max-width:<%= width %>px;max-height:<%= height %>px"><% if (key instanceof Object) { %><img class="img-responsive" src="<% if (!key.value.match(/^https?:/)) { %><%= prefix %><% } %><%= key.value %><%= suffix %>" alt="<%= key.title %>" /></a><% } else { %><img class="img-responsive" src="<% if (!key.match(/^https?:/)) { %><%= prefix %><% } %><%= key %><%= suffix %>" alt="" /><% } %></a> <% }); %>' +
        '</div>' +
      ...
      $(node.el).on('click', '.dropdown-menu a', function (evt) {
        ...
        if (value) {
          ...
          $(node.el).find('a[data-toggle="dropdown"]')
            ...
            .html('<img class="img-responsive" src="' + (!value.match(/^https?:/) ? prefix : '') + value + suffix + '" alt="" />');
        }
        else {
  ...
  'tabarray': {
    ...
    'onInsert': function (evt, node) {
      ...
      var updateTabs = function (selIdx) {
        ...
        if (activateFirstTab) {
          $('> .jsonform-tabbable > .nav-tabs [data-idx="0"]', $nodeid).addClass('active');
        }

...
jsonform.elementTypes = {
  ...
  'radiobuttons': {
    'template': '<div id="<%= node.id %>">' +
      '<% _.each(node.options, function(key, val) { %>' +
        '<label class="<%= cls.buttonClass %> <% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %>active <%= elt.activeClass ? " " + elt.activeClass : "" %><% } %>">' +
        '<input type="radio" style="position:absolute;left:-9999px;"<%= (fieldHtmlClass ? " class=\'" + fieldHtmlClass + "\'": "") %> ' +
  ...
  'checkboxes':{
    ...
    'childTemplate': function(inner, node) {
      ...
      template += '<input ' +
        ...
        '<%= value !== undefined && value !== null && value !== "" ? "checked=\'checked\' " : "" %>' +
        '<%= (node.disabled || node.isReadOnly() ? "disabled " : "") %>';
  ...
  'checkboxbuttons': {
    ...
    'onBeforeRender': function (data, node) {
      ...
      var template = '<label class="<%= cls.buttonClass %>">' +
        ...
        '<% if (checked) { %> checked="checked" <% } %> name="<%= name %>" value="<%= value %>" <%= (node.disabled || node.isReadOnly() ? "disabled" : "") %>' +
    ...
    'template': '<div id="<%= node.id %>">' +
      '<% _.each(node.options, function(key, val) { %>' +
        '<% if (elt.optionsType === "radiobuttons") { %>' +
          '<label class="<%= cls.buttonClass %> <%= ((key instanceof Object && key.htmlClass) ? " " + key.htmlClass : "") %>">' +
            '<input ' +

...
jsonform.elementTypes = {
    ...
    'onInsert': function(evt, node) {
      if (global.Picker) {
        ...
        $(node.el).find('#' + escapeSelector(node.id)).spectrum({
          ...
          showAlpha: false,
          allowEmpty: true,
          disabled: node.disabled || node.isReadOnly() ? true : false,
          preferredFormat: "hex",
          showInput: true
        });
      }
  ...
  'radios':{
    'template': '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %>' +
      ...
      '<input<%= (fieldHtmlClass ? " class=\'" + fieldHtmlClass + "\'": "") %> type="radio" <% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %> checked="checked" <% } %> name="<%= node.name %>" value="<%= (key instanceof Object ? key.value : key) %>"' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.required ? " required=\'required\'" : "") %>' +
      '/><%= (key instanceof Object ? key.title : key) %></label><%= elt.inline ? "" : "</div>" %> <% }); %></div>',
    'fieldtemplate': true,
    'inputfield': true,
    'onInsert': function (evt, node) {

...
var initializeTabs = function (tabs) {
  var activate = function (element, container) {
    container
      .find('> .active')
      .removeClass('active').find('a').removeClass('active');
...
jsonform.fieldTemplate = function(inner) {
  return '<div ' +
    ...
    '<% }%>' +
    'class="<%= cls.groupClass %> jsonform-node jsonform-error-<%= keydash %>' +
    ...
    '<% if (!elt.notitle && node.inlinetitle !== true) { %>' +
      '<label class="<%= cls.labelClass %>" for="<%= node.id %>"><%= node.title ? node.title : node.name %></label>' +
    '<% } %>' +
...
var fileDisplayTemplate = '<div class="_jsonform-preview">' +
  ...
  '<% } %>' +
  '</div>' +
  '<a href="#" class="<%= cls.buttonClass %> _jsonform-delete"><i class="<%= cls.iconClassPrefix %>-trash" title="Remove"></i></a> ';
...
var numberFieldTemplate = function (type, isTextualInput) {
  return {
    'template': ((type === "range") ? '<div class="range">' : '') +
      '<input type="' + type + '" ' +
      'class="<%= fieldHtmlClass' + (isTextualInput ? ' || cls.textualInputClass' : '') + ' %>' + ((type === "range") ? 'form-control-range' : '') + '" ' +
      'name="<%= node.name %>" value="<%= escape(value) %>" id="<%= id %>"' +
      ' aria-label="<%= node.title ? escape(node.title) : node.name %>"' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.isReadOnly() ? " readonly=\'readonly\'" : "") %>' +
...
jsonform.elementTypes = {
  ...
  'checkbox':{
    'template': '<div class="form-check">' +
      '<input ' +
        'class=\'form-check-input<%= (checkboxTitle ? "" : " position-static") %><%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>\' ' +
        'type="checkbox" ' +
        'id="<%= id %>" ' +
        'value="1" ' +
        'name="<%= node.name %>" ' +
  ...
  'imageselect': {
    'template': '<div>' +
      ...
      '<div class="dropdown">' +
      '<a class="<%= cls.buttonClass %>" data-toggle="dropdown" href="#"<% if (node.value) { %> style="max-width:<%= width %>px;max-height:<%= height %>px"<% } %>>' +
      ...
      '<div class="dropdown-menu" id="<%= node.id %>_dropdown">' +
        ...
        '</div>' +
        '<div class="<%= cls.placementClassPrefix %>-right"><a class="<%= cls.buttonClass %>">Reset</a></div>' +
      '</div>' +
      '</div>' +
      '</div>',
    'fieldtemplate': true,
  ...
  'iconselect': {
    'template': '<div>' +
      '<input type="hidden" name="<%= node.name %>" id="<%= node.id %>" value="<%= value %>" />' +
      '<div class="dropdown">' +
        '<a class="<%= cls.buttonClass %>" data-toggle="dropdown" href="#">' +
  ...
  'radios':{
    'template': '<div id="<%= node.id %>">' +
      ...
      '<div class="form-check<% if (elt.inline) { %> form-check<%= cls.inlineClassSuffix %><% } %>">' +
        '<input ' +
          'class=\'form-check-input<%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>\' ' +
          'type="radio" ' +
  ...
  'radiobuttons': {
    'template': '<div id="<%= node.id %>" class="btn-group btn-group-toggle" data-toggle="buttons">' +
      '<% _.each(node.options, function(key, val) { %>' +
      '<label class="<%= cls.buttonClass %><% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %> active<%= elt.activeClass ? " " + elt.activeClass : "" %><% } %><%= (node.disabled || node.isReadOnly() ? " disabled" : "") %>">' +
        '<input ' +
          'type="radio" ' +
  ...
  'checkboxes':{
    ...
    'childTemplate': function(inner, node) {
      ...
      if (!node.formElement.otherField)
        return inner;
      // wrapping div begin
      var template = '<div class="form-check' + (node.formElement.inline ? ' form-check<%= cls.inlineClassSuffix %>' : '') + (node.formElement.otherField.inline ? ' inline-' : ' ') + 'other-field">';
      ...
      template += '/>';
      // label
      template += '<label class="form-check-label" for="' + otherFieldName + '"><%= node.title || "Other" %></label>';
      // end wrapping div and insert other field content
      if (node.formElement.otherField.inline) {
        // put the other field just after the checkbox, wrapped in the div tag
        template += '<div class="other-field-content">'+ inner + '</div>';
        template += '</div>';
      } else {
        // put the other field just after the checkbox's div
        template += '</div>';
        template += '<div class="other-field-content">'+ inner + '</div>';
      }
      return template;
    ...
    'onBeforeRender': function (data, node) {
      ...
      var template = '<div class="form-check' + (node.formElement.inline ? " form-check"+data.cls.inlineClassSuffix : "") + '">' +
        '<input ' +
          'class=\'form-check-input<%= (title ? "" : " position-static") %><%= (node.formElement.fieldHtmlClass ? " " + node.formElement.fieldHtmlClass : "") %>\' ' +
          'type="checkbox" ' +
          'id="<%= name %>" ' +
          'value="<%= escape(value) %>" ' +
          'name="<%= name %>" ' +
  ...
  'checkboxbuttons': {
    'template': '<div id="<%= node.id %>" class="btn-group btn-group-toggle" data-toggle="buttons"><%= choiceshtml %></div>',
    'fieldtemplate': true,
    'inputfield': true,
    'onBeforeRender': function (data, node) {
      ...
      var template = '<label class="<%= cls.buttonClass %><% if (checked) { %> active<%= node.formElement && node.formElement.activeClass ? " " + node.formElement.activeClass : "" %><% } %><%= (node.disabled || node.isReadOnly() ? " disabled" : "") %>">' +
    ...
    'onInsert': function (evt, node) {
      var elt = node.formElement || {};
      var activeClass = elt.activeClass || '';
      $(node.el).find('label').on('click', function () {
  ...
  'array': {
    'template': '<div id="<%= id %>"><ul class="_jsonform-array-ul" style="list-style-type:none;"><%= children %></ul>' +
      '<% if (!node.isReadOnly()) { %><span class="_jsonform-array-buttons">' +
        '<a href="#" class="<%= cls.buttonClass %> _jsonform-array-addmore"><i class="<%= cls.iconClassPrefix %>-plus" title="Add new"></i></a> ' +
        '<a href="#" class="<%= cls.buttonClass %> _jsonform-array-deletelast"><i class="<%= cls.iconClassPrefix %>-minus" title="Delete last"></i></a>' +
      '</span><% } %>' +
      '</div>',
    'fieldtemplate': true,
    'array': true,
    'childTemplate': function (inner, node) {
      if (!node.isReadOnly() && $('').sortable) {
        ...
        return '<li data-idx="<%= node.childPos %>">' +
          ...
          ((!isSet(node.formElement.draggable) || node.formElement.draggable) ? '<span class="draggable line"><i class="<%= cls.iconClassPrefix %>-list" title="Move item"></i></span> ' : '') +
          '<a href="#" class="_jsonform-array-item-delete"><i class="<%= cls.iconClassPrefix %>-trash" title="Remove item"></i></a>' +
          inner +
          '</li>';
      }
      else {
    ...
    'onInsert': function (evt, node) {
      ...
      $('> span > a._jsonform-array-addmore', $nodeid).on("click", function (evt) {
        evt.preventDefault();
        evt.stopPropagation();
      ...
      $('> span > a._jsonform-array-deletelast', $nodeid).on("click", function (evt) {
  ...
  'tabarray': {
    ...
    'onInsert': function (evt, node) {
      ...
      var updateTabs = function (selIdx) {
        ...
        if (selIdx === undefined) {
          selIdx = $('> .jsonform-tabbable > .nav-tabs .active', $nodeid).data('idx');
          if (selIdx) {
        ...
        _.each(node.children, function (child, idx) {
          ...
          tabs += '<li class="nav-item" data-idx="' + idx + '">' +
                  '<a class="draggable tab nav-link" data-toggle="tab" rel="' + escape(title) + '">' +
                  escapeHTML(title);
          if (!node.isReadOnly()) {
            tabs += ' <span href="#" class="_jsonform-array-item-delete"><i class="' +
              node.ownerTree.defaultClasses.iconClassPrefix + '-trash" title="Remove item"></i></span>' +
        ...
        if (!node.isReadOnly() && (boundaries.maxItems < 0 || node.children.length < boundaries.maxItems)) {
          tabs += '<li class="nav-item" data-idx="-1"><a class="tab nav-link _jsonform-array-addmore" title="'+(node.formElement.addMoreTooltip ? escapeHTML(node.formElement.addMoreTooltip) : 'Add new item')+'"><i class="' +
            node.ownerTree.defaultClasses.iconClassPrefix + '-plus"></i> '+(node.formElement.addMoreTitle || 'New')+'</a></li>';
        }
        $('> .jsonform-tabbable > .nav-tabs', $nodeid).html(tabs);
        var canDelete = boundaries.minItems >= 0 && node.children.length <= boundaries.minItems;
        $nodeid.find('> .jsonform-tabbable > .nav-tabs > li > a > ._jsonform-array-item-delete').toggle(!canDelete);
        if (activateFirstTab) {
          $('> .jsonform-tabbable > .nav-tabs [data-idx="0"]', $nodeid).addClass('active').find('a').addClass('active');
        }
        $('> .jsonform-tabbable > .nav-tabs [data-toggle="tab"]', $nodeid).eq(selIdx).trigger("click");
      };
      ...
      var deleteItem = function (idx) {
        ...
        $nodeid.find('> a._jsonform-array-deleteitem')
          .toggleClass('disabled', itemNumCanDelete <= 1);
        $nodeid.find('> .jsonform-tabbable > .nav-tabs > li > a > ._jsonform-array-item-delete').toggle(itemNumCanDelete > 1);
        if (itemNumCanDelete < 1) {
          return false;
        }
      ...
      var addItem = function (idx) {
        ...
        node.insertArrayItem(idx,
          $nodeid.find('> .jsonform-tabbable > .tab-content').get(0));
        updateTabs(idx);
      ...
      $('> a._jsonform-array-deleteitem', $nodeid).on("click", function (evt) {
        var idx = $('> .jsonform-tabbable > .nav-tabs .active', $nodeid).data('idx');
        evt.preventDefault();
        evt.stopPropagation();
        deleteItem(idx);
      });

      //$('> a._jsonform-array-addmore, > .jsonform-tabbable > .nav-tabs > li > ._jsonform-array-addmore', $nodeid).on('click', function (evt) {
      $nodeid.on('click', '> a._jsonform-array-addmore, > .jsonform-tabbable > .nav-tabs > li > ._jsonform-array-addmore', function (evt) {
        var idx = node.children.length;
        evt.preventDefault();
        evt.stopPropagation();
        addItem(idx);
      ...
      $nodeid.on('click', '> .jsonform-tabbable > .nav-tabs > li > a > ._jsonform-array-item-delete', function (e) {
        e.preventDefault();
        e.stopPropagation();
      ...
      if (!isSet(node.formElement.draggable) || node.formElement.draggable) {
        if (!node.isReadOnly() && $(node.el).sortable) {
          $('> .jsonform-tabbable > .nav-tabs', $nodeid).sortable({
            containment: node.el,
  ...
  'fieldset': {
    'template': '<fieldset class="jsonform-node jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
      ...
      '<% if (node.title || node.legend) { %><legend class="<%= cls.legendClass %>" role="treeitem" aria-expanded="false"><%= node.title || node.legend %></legend><% } %>' +
  ...
  'advancedfieldset': {
    'template': '<fieldset class="jsonform-node jsonform-error-<%= keydash %> <% if (elt.expandable !== false) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>"' +
      ...
      '<% if (elt.expandable !== false) { %><div class="<%= cls.groupClass %>"><% } %>' +
      '<%= children %>' +
  ...
  'submit':{
    'template':'<input type="submit" <% if (id) { %> id="<%= id %>" <% } %> class="<%= cls.submitButtonClass %> <%= elt.htmlClass?elt.htmlClass:"" %>" value="<%= value || node.title %>"<%= (node.disabled? " disabled" : "")%>/>'
  },
  'reset':{
    'template':'<input type="reset" <% if (id) { %> id="<%= id %>" <% } %> class="<%= cls.resetButtonClass %> <%= elt.htmlClass?elt.htmlClass:"" %>" value="<%= value || node.title %>"<%= (node.disabled? " disabled" : "")%>/>'
  },
  'button':{
    ...
    'onBeforeRender': function (data, node) {
      ...
      _.each(node.children, function (child, idx) {
        ...
        tab_list += '<li class="nav-item' +
          ((idx === 0) ? ' active' : '') + '">' +
          '<a href="#'+ parentID + title_escaped +'" class="nav-link' + ((idx === 0) ? ' active' : '') + '"' +
    ...
    'template': '<fieldset class="tab-container <%= elt.htmlClass?elt.htmlClass:"" %>"<%= (node.disabled ? " disabled" : "") %>>' +
      ...
      '<div class="jsonform-tabbable">' +
        '<div class="<%= cls.groupClass %><%= node.formElement.hideMenu ? " hide" : "" %>">' +
          '<% if (!elt.notitle) { %><label class="<%= cls.labelClass %>" for="<%= node.id %>"><%= node.title ? node.title : node.name %></label><% } %>' +
   ...
   */
  'question': {
    'template': '<div id="<%= node.id %>"<% if (elt.optionsType === "radiobuttons") { %> class="btn-group btn-group-toggle" data-toggle="buttons"<% } %>>' +
      '<% _.each(node.options, function(key, val) { %>' +
        '<% if (elt.optionsType === "radiobuttons") { %>' +
          '<label class="<%= cls.buttonClass %><%= ((key instanceof Object && key.htmlClass) ? " " + key.htmlClass : "") %><%= (node.disabled || node.isReadOnly() ? " disabled" : "") %>">' +
            '<input ' +
              'type="radio" ' +
              'style="position:absolute;left:-9999px;" ' +
              'name="<%= node.id %>" ' +
        ...
        '<% } else { %>' +
          '<div class="form-check<% if (elt.inline) { %> form-check<%= cls.inlineClassSuffix %><% } %>">' +
            '<input ' +
              'class=\'form-check-input<%= ((key instanceof Object && key.htmlClass) ? " " + key.htmlClass : "") %>\' ' +
              'type="radio" ' +
              ...
              '<%= (node.disabled || node.isReadOnly() ? "disabled" : "") %>' +
            '/>' +
            '<label class="form-check-label" for="<%= node.id %>_<%= escape(val) %>">' +
              '<%= (key instanceof Object ? key.title : key) %>' +
            '</label>' +
          '</div>' +
        '<% } %>' +
  ...
  if (this.view && this.view.inputfield && this.schemaElement) {
    ...
    if (values) {
      ...
      if (previousArrayValue && Array.isArray(previousArrayValue)) {
        ...
        // with disappearing one empty item in rendered array while sorting but
        // breaks rule related to submitted forms without childrens - which I
        // find more natural from a user experience perspective
    ...
    // compute initial values from that object
      ...
      if (minItems >= 0) {
        // fix problems related to tabarray and minItems
        nbChildren = 0;
      }

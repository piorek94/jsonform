...
jsonform.elementTypes = {
  ...
  'switch':{
    'template': '<div class="custom-control custom-switch">' +
      '<input ' +
        'class=\'custom-control-input<%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>\' ' +
        'type="checkbox" ' +
        'id="<%= id %>" ' +
        'value="1" ' +
        'name="<%= node.name %>" ' +
  ...
  // },
  'select':{
    'template': '<div<% if (cls.selectWrapClass) { %> class="<%= cls.selectWrapClass %>"<% } %>>' +
    ...
    'childTemplate': function (inner, node) {
      if (!node.isReadOnly()) {
        ...
        return '<li data-idx="<%= node.childPos %>">' +
          ...
          (($('').sortable && (!isSet(node.formElement.draggable) || node.formElement.draggable)) ? '<span class="draggable line"><i class="<%= cls.iconClassPrefix %>-list" title="Move item"></i></span> ' : '') +
  ...
  'tabarray': {
    ...
    'onInsert': function (evt, node) {
      var $nodeid = $(node.el).find('#' + escapeSelector(node.id));
      var boundaries = node.getArrayBoundaries();
      var addMoreSelector = '> .jsonform-tabbable > .nav-tabs > li > ._jsonform-array-addmore';
      var itemDeleteSelector = '> .jsonform-tabbable > .nav-tabs > li > a > ._jsonform-array-item-delete';
  ...
  'selectfieldset': {
    'template': '<fieldset class="tab-container <%= elt.htmlClass?elt.htmlClass:"" %>"<%= (node.disabled ? " disabled" : "") %>>' +
      ...
      '<div class="jsonform-tabbable">' +
        '<div class="<%= cls.formClass %><%= node.formElement.hideMenu ? " hide" : "" %>">' +
    ...
    'onBeforeRender': function (data, node) {
      ...
      var elt = node.formElement;
      var tabs = '<div ' + (data.cls.selectWrapClass ? ' class="' + data.cls.selectWrapClass + '"' : '') + '>' +
        '<select class="nav '+ (data.cls.textualInputClass) + '"' +
        (node.disabled ? ' disabled' : '') +
        '>';
      _.each(children, function (child, idx) {

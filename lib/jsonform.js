...
jsonform.elementTypes = {
  ...
  'iconselect': {
    'template': '<div>' +
      '<input type="hidden" name="<%= node.name %>" id="<%= node.id %>" value="<%= value %>" />' +
      '<div class="dropdown">' +
      '<a class="<%= node.value ? buttonClass : cls.buttonClass %>" data-toggle="dropdown" href="#">' +
        '<% if (node.value) { %><i class="<%= cls.iconClassPrefix %>-<%= node.value %>" alt="" /><% } else { %><%= buttonTitle %><% } %>' +
      '</a>' +
      '<div class="dropdown-menu" id="<%= node.id %>_dropdown">' +
        '<div>' +
        '<% _.each(node.options, function(key, idx) { if ((idx > 0) && ((idx % columns) === 0)) { %></div><div><% } %><a class="<%= cls.buttonClass %>" ><% if (key instanceof Object) { %><i class="<%= cls.iconClassPrefix %>-<%= key.value %>" alt="<%= key.title %>" /></a><% } else { %><i class="<%= cls.iconClassPrefix %>-<%= key %>" alt="" /><% } %></a> <% }); %>' +
        '</div>' +
        '<div class="<%= cls.placementClassPrefix %>-right"><a class="<%= cls.buttonClass %>">Reset</a></div>' +
      '</div>' +
      '</div>' +

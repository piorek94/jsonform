...
(function(serverside, global, $, _, JSON) {
  ...
  $.fn.jsonFormErrors = function(errors, options) {
    ...
    // XXX: check not expanded expandable fieldsets if error happens there
    var $errorInvisibleFieldsets = $errorSelectors.parents('fieldset.expandable').not('.expanded');
    var $errorFieldsets = $();
    $errorInvisibleFieldsets.each(function() {
      var $this = $(this);
      $errorFieldsets = $errorFieldsets.add($this.addClass(form.defaultConfig.groupMarkClassPrefix + 'error'));
    });
    ...
    if (!firstError && $errorFieldsets.length > 0) {
      firstError = $errorFieldsets.get(0);
    }
